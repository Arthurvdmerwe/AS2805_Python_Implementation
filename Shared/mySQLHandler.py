...
class mySQLHandler(logging.Handler):
    ...
    insertion_sql = """INSERT INTO logs_host(
                        created,
                        name,
                        log_level,
                        log_level_name,
    ...
    def emit(self, record):
        ...
        try:
            ...
            try:            # try to recreate table
                cur.execute(mySQLHandler.insertion_sql)
...
def main():
    def print_all_log(oLog):
        ...
        Async(oLog.debug('debug'))
        Async(oLog.info('info'))
        Async(oLog.warning('warning'))
        Async(oLog.error('error'))
        Async(oLog.critical('critical'))
